...
public class BasePlayer implements Player {
    ...
    //Suiveur - Suivi
    public Map<Integer, BasePlayer> follower = new HashMap<>();
    public BasePlayer follow = null;
    //Prison Alignement :
    ...
    private final Map<Integer, Integer> spellPositions; // K: SpellID, V: Position
    ...
    public BasePlayer(int id, String name, int groupe, int sexe, int classe,
                      ...
                      int morphMode, String allTitle, String emotes, long prison,
    ...
    public BasePlayer(int id, String name, int groupe, int sexe, int classe,
        ...
        this.spellPositions = new HashMap<>(spellPositions);
        try {
        ...
        try {
            ...
            this.morphMode = World.world.getMorphMode(morphMode);
    ...
    public String encodeSpellsToDB() {
        ...
        Map<Integer, Integer> positions = spellPositions;
...
//        }
        ...
        Map<Integer, Spell.SortStats> spells = _sorts;
        Map<Integer, Integer> spellPositions = this.spellPositions;
    ...
    public void learnSpell(int spell, int level, int pos) {
        ...
        if (!_sorts.containsKey(spell)) {
            ...
            spellPositions.remove(spell);
            spellPositions.put(spell, pos);
            SocketManager.GAME_SEND_SPELL_LIST(this);
            SocketManager.GAME_SEND_Im_PACKET(this, "03;" + spell);
        }
    }
    ...
    public boolean unlearnSpell(int spell) {
        ...
        _sorts.remove(spell);
        this.spellPositions.remove(spell);
        SocketManager.GAME_SEND_SPELL_LIST(this);
        SocketManager.GAME_SEND_STATS_PACKET(this);
        ((PlayerData) DatabaseManager.get(PlayerData.class)).update(this);
        return true;
    ...
    @Override
    public boolean unlearnSpell(BasePlayer perso, int spellID, int level,
    ...
    public String encodeSpellListForSL() {
        return getSpells().stream().map(s -> {
            // Official servers send position 126 for spells without shortcuts
            int pos = Optional.ofNullable(getSpellPositions().get(s.getSpellID())).orElse(126);
            return String.join("~",
    ...
    public void setSpellShortcuts(int spellId, int position) {
        ...
        spellPositions.remove(spellId);
        if(position <= 30) spellPositions.put(spellId, position);
        ((PlayerData) DatabaseManager.get(PlayerData.class)).update(this);
    }

    private void removeSpellShortcutAtPosition(int position) {
        for (int key : _sorts.keySet())
            if (spellPositions.get(key) != null)
                if (spellPositions.get(key).equals(position))
                    spellPositions.remove(key);
    }
    ...
    public void OnJoinGame() {
        ...
        if(Config.modeHeroic) {
            this.alignment = 0;
            Optional<BasePlayer> p = new ArrayList<>(World.world.getOnlinePlayers()).stream().filter(p1 -> p1 != null && p1.getAlignment() > 0
        ...
        sendItemShortcuts();
        SocketManager.GAME_SEND_EMOTE_LIST(this, Player.getCompiledEmote(this.emotes));
    ...
    public void teleportLaby(short newMapID, int newCellID) {
        ...
        {
            for (BasePlayer t : this.follower.values()) {
                if (t.isOnline())
    ...
    public void teleport(GameMap map, int cell) {
        ...
        {
            for (BasePlayer t : follower.values()) {
                if (t.isOnline())
                    SocketManager.GAME_SEND_FLAG_PACKET(t, this);
                else
    ...
    @Override
    public boolean isDispo(BasePlayer sender) {
    ...
    public String get_wife_friendlist() {
        BasePlayer wife = World.world.getPlayer(this.wife);
        StringBuilder str = new StringBuilder();
        if (wife != null) {
    ...
    @Override
    public void meetWife(BasePlayer p)// Se teleporter selon les sacro-saintes autorisations du mariage.
    {
        if (p == null)
    ...
    public void revive() {
        ...
        if(revive == 1) {
            ...
            this.spellPositions.putAll(Constant.getStartSortsPlaces(classe));
            if(this.level >= 100)
    ...
    @Override
    public void leaveEnnemyFactionAndPay(BasePlayer perso) {
        if (!isInEnnemyFaction)
            return;//pas en prison on fait pas la commande
            ...
            if ((Integer) this.getExchangeAction().getValue() != 0) {
                ...
                for (BasePlayer z : World.world.getGuild(collector.getGuildId()).getPlayers()) {
                    ...
                    if (z.isOnline()) {
                        ...
                        BasePlayer.this.getGuildMember().giveXpToGuild(collector.getXp());
    ...
    public boolean addStaticEmote(int emote) {
        ...
        if (!isOnline())
            return true;
        SocketManager.GAME_SEND_EMOTE_LIST(this, Player.getCompiledEmote(getEmotes()));
        SocketManager.GAME_SEND_STATS_PACKET(this);
    ...
    public void refreshCraftSecure(boolean unequip) {
        for (BasePlayer player : this.getCurMap().getPlayers()) {
            ...
            if (jobs != null) {
                ...
                if (object == null) {
                    if (unequip) {
                        for(BasePlayer target : this.getCurMap().getPlayers())
