...
// Temporary class, will be removed once items are refactored
public class FullItem implements Item {
    ...
    public FullItem(int Guid, int template, int qua, int pos, String strStats, int puit) {
    ...
    public FullItem(int Guid) {
        this.scriptVal = new SItem(this);
        this.guid = Guid;
        this.templateID = 8378;
        this.quantity = 1;
        this.position = -1;
        this.puit = 0;
    }

    public FullItem(int Guid, int template, int qua, int pos, Stats stats, ArrayList<SpellEffect> effects, Map<Integer, Integer> _SoulStat, Map<Integer, String> _txtStats, int puit) {
        this.scriptVal = new SItem(this);
        this.guid = Guid;
        this.templateID = template;
        this.quantity = qua;
    ...
    public FullItem getClone(int qua, boolean insert) {
        Map<Integer, Integer> maps = new HashMap<>(this.getStats().getEffects());
        ...
        FullItem object = new FullItem(-1, this.templateID, qua, insert ? Constant.ITEM_POS_NO_EQUIPED : this.getPosition(), newStats, effects, this.getSoulStat(), this.getTxtStat(), this.getPuit());
        if(insert)
            ...
            for (String split : strStats.split(",")) {
                try {
                    if (split.equalsIgnoreCase(""))
                        continue;
                    if (split.substring(0, 3).equalsIgnoreCase("325") && (this.templateID == 10207 || this.templateID == 10601)) {
                    ...
                    if (id == Constant.STATS_PETS_DATE && typeID() == Constant.ITEM_TYPE_CERTIFICAT_CHANIL) {
                    ...
                    if (this.templateID == 77 && id == Constant.STATS_PETS_DATE) {
                        txtStats.put(id, split.substring(3));
                        continue;
                    }
                    ...
                    if (id != Constant.STATS_RESIST && (!stats[3].isEmpty() && (!stats[3].equals("0") || id == Constant.STATS_PETS_DATE || id == Constant.STATS_PETS_PDV || id == Constant.STATS_PETS_POIDS || id == Constant.STATS_PETS_EPO || id == Constant.STATS_PETS_REPAS))) {//Si le stats n'est pas vide et (n'est pas �gale � 0 ou est de type familier)
                        if (!(typeID() == Constant.ITEM_TYPE_CERTIFICAT_CHANIL && id == Constant.STATS_PETS_DATE)) {
                    ...
                    if (id == Constant.STATS_RESIST && typeID() == 93) {
                        txtStats.put(id, stats[4]);
                        continue;
                    }
    ...
    public void setQuantity(int quantity) {
        if (quantity <= 0)
            ...
            if (Logging.USE_LOG)
                Logging.getInstance().write("Object", "Faille : Objet guid : " + guid + " a dépassé 100 000 qua (" + quantity + ") avec comme template : " + template().getName() + " (" + templateID + ")");
    ...
    public Mount setMountStats(Player player, Mount mount, boolean castrated) {
        if(mount == null)
            mount = new Mount(Constant.getMountColorByParchoTemplate(this.templateID), player.getId(), false);
    ...
    public String encodeItem() {
        ...
        return Integer.toHexString(guid) + "~"
                + Integer.toHexString(templateID) + "~"
                + Integer.toHexString(quantity) + "~" + posi + "~"
                + encodeStats() + ";";
    }

    public String encodeStats() {
        if (typeID() == 83) //Si c'est une pierre d'�me vide
            return this.template().getStrTemplate();

        final StringBuilder stats = new StringBuilder();
        boolean isFirst = true;

        // Panoplie de classe (81 à 92)
        if (this.template().getPanoId() >= 81 && this.template().getPanoId() <= 92) {
        ...
        for (Entry<Integer, String> entry : txtStats.entrySet()) {
            ...
            if (typeID() == 77 || typeID() == 90) {
                ...
                if (entry.getKey() == Constant.STATS_PETS_DATE && type() == ItemType.KENNEL_CERTIFICATE) {
                ...
                PetEntry p = World.world.getPetsEntry_legacy(this.getGuid());
                if (p == null) {
                    if (entry.getKey() == Constant.STATS_PETS_PDV)
                    ...
                    && type().clientID == 93) {
                stats.append(Integer.toHexString(entry.getKey())).append("#").append(Integer.toHexString(getResistanceMax(template().getStrTemplate()))).append("#").append(entry.getValue()).append("#").append(Integer.toHexString(getResistanceMax(template().getStrTemplate())));
            } else if (entry.getKey() == Constant.STATS_RESIST) {
                stats.append(Integer.toHexString(entry.getKey())).append("#").append(Integer.toHexString(getResistanceMax(template().getStrTemplate()))).append("#").append(entry.getValue()).append("#").append(Integer.toHexString(getResistanceMax(template().getStrTemplate())));
            } else {
                stats.append(Integer.toHexString(entry.getKey())).append("####").append(entry.getValue());
        ...
        for (Entry<Integer, Integer> entry : this.stats.getEffects().entrySet()) {
            ...
            if ((template().getPanoId() >= 81 && template().getPanoId() <= 92)
                    || (template().getPanoId() >= 201 && template().getPanoId() <= 212)) {
    ...
    public String parseStatsStringSansUserObvi() {
        if (type().clientID == 83) //Si c'est une pierre d'�me vide
        ...
        for (Entry<Integer, String> entry : txtStats.entrySet()) {
            if (!isFirst)
                stats.append(",");
            if (type() == ItemType.KENNEL_CERTIFICATE) {
                if (entry.getKey() == Constant.STATS_PETS_PDV)
    ...
    public String obvijevanOCO_Packet(int pos) {
        ...
        upPacket = upPacket + Integer.toHexString(templateID) + "~";
    ...
    public String getObvijevanStatsOnly() {
        FullItem obj = this.getClone(1, true);
    ...
    /** FM TOUT POURRI **/
    public String parseStringStatsEC_FM(FullItem obj, double poid, int carac) {
        String stats = "";
        boolean first = false;
        double perte = 0.0;
        ...
        for (Integer i : keys) {
            ...
            if (perte > poid || statID == carac) {
                ...
                if (newstats > JobAction.getBaseMaxJet(obj.templateID, Integer.toHexString(i)))
                    newstats = JobAction.getBaseMaxJet(obj.templateID, Integer.toHexString(i));
    ...
    public String parseFMStatsString(String statsstr, FullItem obj, int add,
                                     boolean negatif) {
        String stats = "";
        boolean isFirst = true;
    ...
    public boolean isSameStats(FullItem newObj) {
    ...
    public int templateID() {
        // If it's a pet and it's dead, change template ID
        // this is a temporary solution until items are reworked
        return World.world.getPetsEntry(this.guid).map(pe -> {
            if(pe.getPdv() > 0) return templateID;
